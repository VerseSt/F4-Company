...
zeroyt7.on("group-participants-update", async (anu) => {
    ...
    try {
      ...
      if (anu.action == "add" && mem.includes(zeroyt7.user.jid)) {
        zeroyt7.sendMessage(anu.jid, "Halo!.. saya F4-X saya akan membatu mempermudah kehidupan..seperti membuat sticker dan lain-lain. untuk memulai silahkan ketik !menu.", "conversation")
      }
      if (!isWelkom) return
      if (anu.action == "add" && !mem.includes(zeroyt7.user.jid)) {
        ...
        buff = await getBuffer(
          `http://hadi-api.herokuapp.com/api/card/welcome?nama=${anu_user}&descriminator=${groupMembers.length
            ...
            mdata.subject
          )}&pp=${pp_user}&https://ibb.co/r3Rwc92`
        )

        but = [
          { buttonId: 'add', buttonText: { displayText: 'Halo Beban Baru😎👋' }, type: 1 }
        ]
        sendButImage(mdata.id, wel, "©F4-X", buff, but)
      }
      if (!isWelkom) return
      if (anu.action == "remove" && !mem.includes(zeroyt7.user.jid)) {
        ...
        buff = await getBuffer(
          `http://hadi-api.herokuapp.com/api/card/goodbye?nama=${anu_user}&descriminator=${groupMembers.length
            ...
            mdata.subject
          )}&pp=${pp_user}&bg=https://ibb.co/r3Rwc92`
        )

        but = [
          { buttonId: 'remove', buttonText: { displayText: 'Byeee Beban👋' }, type: 1 }
        ]
        sendButImage(mdata.id, out, "©F4-X", buff, but)
      }
